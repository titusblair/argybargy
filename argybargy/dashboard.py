...
(function () {
  ...
  /* room name -> {messages, last_seq, last_ts, seconds_since_last} from /admin/state */
  function roomSummaries() {
    var by = {};
    ((S.data && S.data.rooms) || []).forEach(function (r) { by[r.room] = r; });
    return by;
  }
  /* Age of a room's last message, drifting forward between polls so the
     sidebar ticks instead of freezing for 3 seconds at a time. */
  ...
  function roomList() {
    ...
    return Object.keys(set).sort(function (a, b) {
      ...
      if (sa && sb) { return sa.seconds_since_last - sb.seconds_since_last || a.localeCompare(b); }
      if (sa) { return -1; }
      if (sb) { return 1; }
  ...
  /* ------------------------------------------------------------- deep link */
  /* /dashboard?room=<name> opens straight into one stream, and picking a room
     rewrites the URL so the address bar is always bookmarkable. */
  ...
  function syncUrl() {
    var want = S.view.room ? "?room=" + encodeURIComponent(S.view.room) : "";
    if (window.location.search !== want) {
      history.replaceState(null, "", window.location.pathname + want);
    }
  }
  function openRoom(room) {
    S.view = { kind: "room", room: room, agent: null };
    S.stick = true;
    markSeen();
    syncUrl();
  }
  ...
  function renderHeader() {
    ...
    var summary = roomSummaries()[S.view.room];
    if (summary) {
      h.appendChild(E("span", "conv-header__sep"));
      h.appendChild(E("span", "conv-header__count", {
        "data-testid": "room-count",
        title: "messages kept for this room, and the age of the newest one",
        text: summary.messages + " msg · " + lastSeen(roomAge(summary)) + " ago"
      }));
    }
  ...
  /* Ask for the room in view so its own tail comes back, not whatever slice of
     the global tail happens to belong to it. Room summaries come back either way. */
  function poll() {
  ...
  S.view.room = roomFromUrl();   /* /dashboard?room=<name> opens straight into one stream */
  shell();
  wire();
  renderAll();
  poll();
